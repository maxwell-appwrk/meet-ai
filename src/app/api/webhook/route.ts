...
export async function POST(req: NextRequest) {
    ...
    if (eventType === "call.session_started") {
        ...
        const [existingAgent] = await db.select().from(agents).where(
            eq(
                agents.id, existingMeeting.agentId
            )
        );
        ...
        if (!userId || !channelId || !text) {
            return NextResponse.json({ error: "Missing user id, channel id or text" }, { status: 400 });
        }

        const [existingMeeting] = await db.select().from(meetings).where(and(eq(meetings.id, channelId), eq(meetings.status, MeetingStatus.Completed)));
        ...
        if (userId !== existingAgent.id) {
            ...
            const previousMessages = channel.state.messages.slice(-5).filter((message) => message.text && message.text.trim() !== "").map<ChatCompletionMessageParam>((message) => ({
